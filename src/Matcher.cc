...
namespace ORB_SLAM3
{
    ...
    float Matcher::TH_HIGH = 100.0;
    float Matcher::TH_LOW = 50.0;
...
*/
                                           ...
                                           vector<pair<size_t, size_t> > &vMatchedPairs, const bool bOnlyStereo, const bool bCoarse, std::vector<float> *vfDebugInfo) const
    {
        ...
        const float threshold = 2.0*TH_HIGH;
        ...
        for (int realIdx1 = 0; realIdx1 < realDescriptors1.rows; ++realIdx1)
        {
            float bestDist1 = threshold; // the distance must be larger than this threshold
            int bestRealIdx2 = -1;
            for (int realIdx2 = 0; realIdx2 < realDescriptors2.rows; ++realIdx2)
            {
                float dist = DescriptorDistance(realDescriptors1.row(realIdx1), realDescriptors2.row(realIdx2));
            ...
            if(bestRealIdx2 != -1)
            {
                ...
                for(int crossRealIdx1 = 0; crossRealIdx1 < realDescriptors1.rows; crossRealIdx1++)
                {
                    float dist = DescriptorDistance(realDescriptors1.row(crossRealIdx1), realDescriptors2.row(bestRealIdx2));
                ...
                if(bCoarse || pCamera1->epipolarConstrain(pCamera2,kp1,kp2,R12,t12,pKF1->mvLevelSigma2[kp1.octave],pKF2->mvLevelSigma2[kp2.octave])) // MODIFICATION_2
                {
		    //cout << " Matched Distance " << DescriptorDistance(Descriptors1.row(idx1), Descriptors2.row(idx2)) << endl;
                    vMatches12[idx1]=idx2; // vMatches12[idx1]=bestIdx2;
                    nmatches++;
                }
            }
