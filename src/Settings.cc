...
namespace ORB_SLAM3 {
    ...
    template<>
    double Settings::readParameter<double>(cv::FileStorage& fSettings, const std::string& name, bool& found, const bool required){
        cv::FileNode node = fSettings[name];
        if(node.empty()){
            if(required){
                std::cerr << name << " required parameter does not exist, aborting..." << std::endl;
    ...
    bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false) {
        ...
        //Open settings file
        fSettings = cv::FileStorage(configFile, cv::FileStorage::READ);
	...
	readMatcherPerameters(fSettings);
        cout << "\t-Loaded Matcher settings" << endl;
